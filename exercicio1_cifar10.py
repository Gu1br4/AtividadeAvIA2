...
for i, ax in enumerate(axes.flat):
    ...
    ax.axis('off')
plt.suptitle('10 Amostras Aleatorias do CIFAR-10', fontsize=14)
plt.tight_layout()
plt.savefig('outputs/01_amostras_aleatorias.png', dpi=150, bbox_inches='tight')
plt.close()
print('Grafico salvo: outputs/01_amostras_aleatorias.png')


# 2) Pre-processamento e Data Augmentation
...
print(f'Media por canal (R, G, B): {CIFAR10_MEAN}')
...
plt.close()
print('Grafico salvo: outputs/02_data_augmentation.png')


# 3) Divisao dos Dados em treino/validacao/teste

print('\n3) Divisao dos Dados (70% treino / 15% val / 15% teste)')
...
print(f'Validacao: {len(X_val_idx):>6} ({len(X_val_idx)/total*100:.1f}%)')
...
tabela_split = pd.DataFrame({
    ...
    'Validacao (15%)': contagem_por_classe(y_val,   'Validacao'),
...
print('\nAmostras por classe em cada particao:')
...
# 5) Treinamento com Validacao para Melhoria de Desempenho

print('\n5) Treinamento com Validacao')
...
for epoch in range(1, EPOCHS + 1):
    ...
    if no_improve >= PATIENCE:
        print(f'\nEarly stopping na epoca {epoch} (sem melhora por {PATIENCE} epocas).')
        break

best_epoch = history['val_acc'].index(best_val_acc) + 1
print(f'\nMelhor val_acc: {best_val_acc:.4f} (epoca {best_epoch})')

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
ax1.plot(history['train_acc'], label='Treino',    color='steelblue')
ax1.plot(history['val_acc'],   label='Validacao', color='orange')
...
ax2.plot(history['val_loss'],   label='Validacao', color='orange')
ax2.set_title('Perda (Loss) por Epoca')
ax2.set_xlabel('Epoca')
ax2.set_ylabel('Loss')
ax2.legend()
ax2.grid(True, alpha=0.3)

plt.suptitle('Curvas de Treino vs. Validacao', fontsize=14)
plt.tight_layout()
plt.savefig('outputs/03_curvas_treino_validacao.png', dpi=150, bbox_inches='tight')
plt.close()
print('Grafico salvo: outputs/03_curvas_treino_validacao.png')
...
print(f'\nAcuracia: {test_acc:.4f} ({test_acc*100:.2f}%)')
...
ax.set_title('Matriz de Confusao - Conjunto de Teste', fontsize=14)
plt.tight_layout()
plt.savefig('outputs/04_matriz_confusao.png', dpi=150, bbox_inches='tight')
plt.close()
print('Grafico salvo: outputs/04_matriz_confusao.png')
...
print('Top 10 pares de confusao (real -> predito):')
